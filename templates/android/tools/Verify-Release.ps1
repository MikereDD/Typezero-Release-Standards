...
foreach ($asset in $manifest.assets) {
    $expectedFileName = ($manifest.displayName + '-v' + $manifest.version + '.apk') -replace ' ', '-'
    if ($asset.fileName -cne $expectedFileName) { throw "APK filename mismatch. Expected $expectedFileName; found $($asset.fileName)" }
    if ($asset.signature.fileName -cne "$($asset.fileName).sig") { throw 'Detached signature filename must equal APK filename plus .sig.' }
    ...
    if ($item.Length -ne [int64]$asset.size) { throw "APK size mismatch: $($asset.fileName)" }
    ...
    & (Join-Path $PSScriptRoot 'Verify-Package-Identity.ps1') `
        -ApkPath $path `
        -ExpectedPackageId $asset.packageId `
        -ApkAnalyzerPath $ApkAnalyzerPath
    & (Join-Path $PSScriptRoot 'Verify-Signing-Certificate.ps1') `
        -ApkPath $path `
        -ExpectedSha256 $asset.signingCertificateSha256 `
